...
func init() {
	...
	cozeloopApiBaseURL = os.Getenv("COZELOOP_API_BASE_URL")
...
func main() {
	...
	if len(langfuseBaseURL) > 0 {
		langfuseHandler, flusher := newLangfuseHandler()
		handlers = append(handlers, langfuseHandler)
		defer flusher()
	}
	if cozeloopApiBaseURL != "" && cozeloopApiToken != "" && cozeloopWorkspaceID != "" {
...
func composeAgent(ctx context.Context,
	...
	if err != nil {
		log.Printf("here some error, %s", err.Error())
		log.Fatal(err)
	}
